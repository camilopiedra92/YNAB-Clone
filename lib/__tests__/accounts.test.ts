...
describe('Account Type', () => {
    it('getAccountType returns correct type', async () => {
        const r1 = await fns.createAccount({ name: 'Checking', type: 'checking' });
        const r2 = await fns.createAccount({ name: 'Visa', type: 'credit' });
    ...
    it('isCreditCardAccount returns true for credit accounts', async () => {
        const result = await fns.createAccount({ name: 'Visa', type: 'credit' });
        const id = result.id;
...
describe('Update Account - Edge Cases', () => {
    it('does nothing with empty updates object', async () => {
        ...
        const updateResult = await fns.updateAccount(id, {});
        expect(updateResult).toBeUndefined();

        const account: any = await fns.getAccount(id);
        expect(account.name).toBe('Test');
    });

    it('updates only name when other fields are undefined', async () => {
        const result = await fns.createAccount({ name: 'Old', type: 'checking' });
        const id = result.id;

        await fns.updateAccount(id, { name: 'New' });
        const account: any = await fns.getAccount(id);
    ...
    it('updates only note', async () => {
        ...
        await fns.updateAccount(id, { note: 'Some note' });
        const account: any = await fns.getAccount(id);
        expect(account.name).toBe('Test');
    ...
    it('opens a closed account', async () => {
        ...
        let account: any = await fns.getAccount(id);
        expect(account.closed).toBe(true);

        await fns.updateAccount(id, { closed: false });
        account = await fns.getAccount(id);
    ...
    it('updates all fields at once', async () => {
        ...
        await fns.updateAccount(id, { name: 'New Name', note: 'New Note', closed: true });
        const account: any = await fns.getAccount(id);
...
describe('Reconciliation', () => {
    it('returns reconciliation info with no transactions', async () => {
        ...
        const info = await fns.getReconciliationInfo(id);
    ...
    it('reconcileAccount only affects currently cleared transactions', async () => {
        ...
        await fns.reconcileAccount(id);

        const reconciled: any = await fns.getTransaction(tx1.id);
        expect(reconciled.cleared).toBe('Reconciled');

        const uncleared: any = await fns.getTransaction(tx2.id);
