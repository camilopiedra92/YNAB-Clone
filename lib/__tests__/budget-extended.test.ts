...
describe('getBudgetForMonth', () => {
    it('returns all non-income categories for a month', async () => {
        ...
        const rows = await fns.getBudgetForMonth(month);
    ...
    it('computes carryforward for categories without budget_months row', async () => {
        ...
        const rows = await fns.getBudgetForMonth(next);
    ...
    it('includes category and group metadata', async () => {
        const { groupId, categoryIds } = await seedBasicBudget(fns);
        const month = currentMonth();
...
describe('getReadyToAssignBreakdown', () => {
    it('returns breakdown with all fields', async () => {
        const { accountId, groupId, categoryIds } = await seedBasicBudget(fns, { accountBalance: 0 });
        const month = currentMonth();
    ...
    it('reports assigned in future months', async () => {
        const { groupId, categoryIds } = await seedBasicBudget(fns, { accountBalance: 0 });
        const month = currentMonth();
        ...
        await fns.updateBudgetAssignment(categoryIds[1], future, mu(500));
    ...
    it('reports cash overspending from previous month', async () => {
        ...
        await fns.updateBudgetAssignment(categoryIds[0], prev, mu(50));
        await fns.createTransaction({
            accountId,
            date: `${prev}-15`,
        ...
        await fns.updateBudgetActivity(categoryIds[0], prev);
...
describe('refreshAllBudgetActivity', () => {
    it('recalculates all categories with transactions', async () => {
        ...
        // Assign budgets
        await fns.updateBudgetAssignment(categoryIds[0], month, mu(500));
        await fns.updateBudgetAssignment(categoryIds[1], month, mu(300));
        ...
        await fns.refreshAllBudgetActivity(month);
...
describe('Credit Card Payment Category', () => {
    ...
    it('ensureCreditCardPaymentCategory reuses existing CC Payments group', async () => {
        const cc1Result = await fns.createAccount({ name: 'Visa', type: 'credit' });
        const cc2Result = await fns.createAccount({ name: 'Mastercard', type: 'credit' });
...
describe('updateCreditCardPaymentBudget', () => {
    it('does nothing if no CC payment category exists', async () => {
        ...
        await fns.updateCreditCardPaymentBudget(99999, currentMonth());
        // Should not throw
    });
    ...
    it('computes funded spending for CC transactions', async () => {
        ...
        await fns.updateCreditCardPaymentBudget(ccId, month);
...
describe('getCashOverspendingForMonth', () => {
    it('returns 0 when no overspending', async () => {
        ...
        const overspending = await fns.getCashOverspendingForMonth(month);
    ...
    it('detects cash overspending', async () => {
        ...
        await fns.updateBudgetAssignment(categoryIds[0], month, mu(50));
        await fns.createTransaction({
            accountId,
            date: today(),
            payee: 'Store',
...
describe('getOverspendingTypes', () => {
    it('returns empty object when no overspending', async () => {
        await seedBasicBudget(fns);
        const result = await fns.getOverspendingTypes(currentMonth());
    ...
    it('classifies cash overspending as cash', async () => {
        ...
        const result = await fns.getOverspendingTypes(month);
...
describe('getBudgetInspectorData', () => {
    it('returns complete inspector data structure', async () => {
        ...
        const data = await fns.getBudgetInspectorData(month);
    ...
    it('reports future month assignments', async () => {
        ...
        await fns.updateBudgetAssignment(categoryIds[0], month, mu(500));
        await fns.updateBudgetAssignment(categoryIds[1], future, mu(300));
    ...
    it('computes reduce overfunding', async () => {
        ...
        await seedCompleteMonth(fns, db, prev, groupId);
        await fns.updateBudgetAssignment(categoryIds[0], prev, mu(500));
    ...
    it('computes reset available amounts', async () => {
        const { categoryIds } = await seedBasicBudget(fns, { accountBalance: 0 });
        const month = currentMonth();
    ...
    it('reports assigned and spent from last month', async () => {
        ...
        await fns.updateBudgetAssignment(categoryIds[0], prev, mu(400));
        await fns.createTransaction({
            accountId,
            date: `${prev}-15`,
            payee: 'Store',
    ...
    it('handles categories with zero available and zero activity in summary', async () => {
        const { groupId } = await seedBasicBudget(fns, { accountBalance: 0 });
        const month = currentMonth();
        ...
        await fns.createCategoryGroup('Empty Group');
