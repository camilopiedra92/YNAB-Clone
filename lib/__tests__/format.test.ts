...
describe('formatCurrency', () => {
    it('formats positive milliunits as COP currency', () => {
    ...
    it('respects custom minimumFractionDigits', () => {
        const result = formatCurrency(1500000, 0);
...
describe('formatCurrencyOrEmpty', () => {
    it('returns empty string for zero', () => {
        expect(formatCurrencyOrEmpty(0)).toBe('');
    });

    it('returns formatted string for non-zero positive', () => {
        const result = formatCurrencyOrEmpty(1000000);
        expect(result).toContain('$');
        expect(result.length).toBeGreaterThan(0);
    });

    it('returns formatted string for non-zero negative', () => {
        const result = formatCurrencyOrEmpty(-500000);
        expect(result.length).toBeGreaterThan(0);
    });
});
