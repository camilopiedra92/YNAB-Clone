import Database from 'better-sqlite3';
import path from 'path';
import fs from 'fs';

const dbPath = path.join(process.cwd(), 'db', 'ynab.db');
const schemaPath = path.join(process.cwd(), 'db', 'schema.sql');

// Ensure db directory exists
const dbDir = path.dirname(dbPath);
if (!fs.existsSync(dbDir)) {
  fs.mkdirSync(dbDir, { recursive: true });
}

// Create or open database
const db = new Database(dbPath);

// Enable foreign keys
db.pragma('foreign_keys = ON');

// Initialize schema if database is new
if (!fs.existsSync(dbPath) || fs.statSync(dbPath).size === 0) {
  const schema = fs.readFileSync(schemaPath, 'utf-8');
  db.exec(schema);
  console.log('Database schema initialized');
}

export default db;

// ====== Factory: all DB functions accept a database instance ======
// This enables tests to inject an in-memory database while production
// code continues to use the file-based singleton above.

export function createDbFunctions(database: Database.Database) {

  function computeCarryforward(categoryId: number, targetMonth: string): number {
    const prev: any = database.prepare(`
      SELECT bm.available
      FROM budget_months bm
      WHERE bm.category_id = ? AND bm.month < ?
      ORDER BY bm.month DESC LIMIT 1
    `).get(categoryId, targetMonth);

    if (!prev || prev.available === 0) return 0;
    if (prev.available > 0) return prev.available;

    // Negative available: only CC Payment categories carry forward debt
    const category: any = database.prepare(
      'SELECT linked_account_id FROM categories WHERE id = ?'
    ).get(categoryId);

    if (category?.linked_account_id) {
      return prev.available; // CC Payment: debt carries forward
    }

    // Regular category: ALL overspending resets to 0 at month rollover
    return 0;
  }

  function getAccounts() {
    return database.prepare('SELECT * FROM accounts ORDER BY name').all();
  }

  function getAccount(id: number) {
    return database.prepare('SELECT * FROM accounts WHERE id = ?').get(id);
  }

  function createAccount(account: {
    name: string;
    type: string;
    balance?: number;
  }) {
    const stmt = database.prepare(`
      INSERT INTO accounts (name, type, balance, cleared_balance, uncleared_balance)
      VALUES (?, ?, ?, ?, ?)
    `);
    const balance = account.balance || 0;
    return stmt.run(account.name, account.type, balance, balance, 0);
  }

  function updateAccount(id: number, updates: {
    name?: string;
    note?: string;
    closed?: boolean;
  }) {
    const fields: string[] = [];
    const values: any[] = [];

    if (updates.name !== undefined) {
      fields.push('name = ?');
      values.push(updates.name);
    }
    if (updates.note !== undefined) {
      fields.push('note = ?');
      values.push(updates.note);
    }
    if (updates.closed !== undefined) {
      fields.push('closed = ?');
      values.push(updates.closed ? 1 : 0);
    }

    if (fields.length === 0) return;

    values.push(id);
    const stmt = database.prepare(`UPDATE accounts SET ${fields.join(', ')} WHERE id = ?`);
    return stmt.run(...values);
  }

  function getCategoryGroups() {
    return database.prepare(`
      SELECT * FROM category_groups 
      ORDER BY sort_order, name
    `).all();
  }

  function getCategories(groupId?: number) {
    if (groupId) {
      return database.prepare(`
        SELECT * FROM categories 
        WHERE category_group_id = ?
        ORDER BY sort_order, name
      `).all(groupId);
    }
    return database.prepare(`
      SELECT c.*, cg.name as group_name 
      FROM categories c
      JOIN category_groups cg ON c.category_group_id = cg.id
      ORDER BY cg.sort_order, c.sort_order, c.name
    `).all();
  }

  function getBudgetForMonth(month: string) {
    const rows: any[] = database.prepare(`
      SELECT 
        CASE WHEN c.id IS NOT NULL THEN COALESCE(bm.id, -c.id) ELSE NULL END as id,
        c.id as category_id,
        c.name as category_name,
        cg.id as category_group_id,
        cg.name as group_name,
        cg.hidden as group_hidden,
        COALESCE(bm.month, ?) as month,
        COALESCE(bm.assigned, 0) as assigned,
        COALESCE(bm.activity, 0) as activity,
        bm.available as available,
        bm.id as _budget_month_id,
        c.linked_account_id
      FROM category_groups cg
      LEFT JOIN categories c ON c.category_group_id = cg.id
      LEFT JOIN budget_months bm ON c.id = bm.category_id AND bm.month = ?
      WHERE cg.is_income = 0
      ORDER BY cg.sort_order, c.sort_order
    `).all(month, month);

    // For categories without a budget_months row, compute carryforward from prior months
    return rows.map((row) => {
      if (row._budget_month_id === null && row.category_id !== null) {
        row.available = computeCarryforward(row.category_id, month);
      } else {
        row.available = row.available ?? 0;
      }
      delete row._budget_month_id;
      return row;
    });
  }

  function getReadyToAssign(month: string): number {
    // 1. Cash on hand: net balance of all budget (non-credit) accounts, excluding future transactions
    const cashBalance: any = database.prepare(`
      SELECT COALESCE(SUM(t.inflow - t.outflow), 0) as total
      FROM transactions t
      JOIN accounts a ON t.account_id = a.id
      WHERE a.type != 'credit' AND t.date <= date('now')
    `).get();

    // 2. Positive CC balances: if a credit card has a positive balance (bank owes user),
    //    that surplus is treated as cash per YNAB rules.
    const positiveCCBalances: any = database.prepare(`
      SELECT COALESCE(SUM(positive_balance), 0) as total
      FROM (
        SELECT MAX(0, COALESCE(SUM(t.inflow - t.outflow), 0)) as positive_balance
        FROM transactions t
        JOIN accounts a ON t.account_id = a.id
        WHERE a.type = 'credit' AND t.date <= date('now')
        GROUP BY a.id
      )
    `).get();

    // 3. Find the latest month with COMPLETE budget data (cumulative available reflects all history)
    const latestMonth: any = database.prepare(`
      SELECT month FROM budget_months
      GROUP BY month
      HAVING COUNT(*) >= 10
      ORDER BY month DESC
      LIMIT 1
    `).get();

    if (!latestMonth?.month) {
      return (cashBalance.total || 0) + (positiveCCBalances.total || 0);
    }

    // 4. Sum of all category available values for the latest month (cumulative)
    const totalAvailable: any = database.prepare(`
      SELECT COALESCE(SUM(bm.available), 0) as total
      FROM budget_months bm
      JOIN categories c ON bm.category_id = c.id
      JOIN category_groups cg ON c.category_group_id = cg.id
      WHERE cg.is_income = 0 AND bm.month = ?
    `).get(latestMonth.month);

    let rta = (cashBalance.total || 0) + (positiveCCBalances.total || 0) - (totalAvailable.total || 0);

    // 4b. Subtract assigned amounts from months BEYOND the latest full month,
    //     but ONLY up to the viewed month M (inclusive).
    const futureAssigned: any = database.prepare(`
      SELECT COALESCE(SUM(bm.assigned), 0) as total
      FROM budget_months bm
      JOIN categories c ON bm.category_id = c.id
      JOIN category_groups cg ON c.category_group_id = cg.id
      WHERE cg.is_income = 0 AND bm.month > ? AND bm.month <= ?
    `).get(latestMonth.month, month);
    rta -= (futureAssigned.total || 0);

    // 5. Credit overspending correction
    const totalOverspending: any = database.prepare(`
      SELECT COALESCE(SUM(ABS(bm.available)), 0) as total
      FROM budget_months bm
      JOIN categories c ON bm.category_id = c.id
      JOIN category_groups cg ON c.category_group_id = cg.id
      WHERE cg.is_income = 0
        AND c.linked_account_id IS NULL
        AND bm.available < 0
        AND bm.month = ?
    `).get(latestMonth.month);

    const cashOverspendingAmount = getCashOverspendingForMonth(latestMonth.month);
    const creditOverspendingAmount = (totalOverspending.total || 0) - cashOverspendingAmount;
    rta -= creditOverspendingAmount;

    // For past months, clamp negative RTA to 0
    const currentMonth = new Date().toISOString().slice(0, 7);
    if (rta < 0 && month < currentMonth) {
      return 0;
    }

    return rta;
  }

  function getReadyToAssignBreakdown(month: string) {
    // Previous month string
    const [y, m] = month.split('-').map(Number);
    const prevDate = new Date(y, m - 2);
    const prevMonth = `${prevDate.getFullYear()}-${String(prevDate.getMonth() + 1).padStart(2, '0')}`;

    // ➕ Inflow: Ready to Assign in current month (cash-based accounts only)
    const inflowThisMonth: any = database.prepare(`
      SELECT COALESCE(SUM(t.inflow - t.outflow), 0) as total
      FROM transactions t
      JOIN accounts a ON t.account_id = a.id
      JOIN categories c ON t.category_id = c.id
      JOIN category_groups cg ON c.category_group_id = cg.id
      WHERE cg.is_income = 1
        AND a.type != 'credit'
        AND strftime('%Y-%m', t.date) = ?
        AND t.date <= date('now')
    `).get(month);

    // ➕ Positive CC balances (cashback / overpayments)
    const positiveCCBalances: any = database.prepare(`
      SELECT COALESCE(SUM(positive_balance), 0) as total
      FROM (
        SELECT MAX(0, COALESCE(SUM(t.inflow - t.outflow), 0)) as positive_balance
        FROM transactions t
        JOIN accounts a ON t.account_id = a.id
        WHERE a.type = 'credit' AND t.date <= date('now')
        GROUP BY a.id
      )
    `).get();

    // ➖ Assigned in current month
    const assignedThisMonth: any = database.prepare(`
      SELECT COALESCE(SUM(bm.assigned), 0) as total
      FROM budget_months bm
      JOIN categories c ON bm.category_id = c.id
      JOIN category_groups cg ON c.category_group_id = cg.id
      WHERE cg.is_income = 0 AND bm.month = ?
    `).get(month);

    // ➖ Assigned in future (months beyond current)
    const assignedInFuture: any = database.prepare(`
      SELECT COALESCE(SUM(bm.assigned), 0) as total
      FROM budget_months bm
      JOIN categories c ON bm.category_id = c.id
      JOIN category_groups cg ON c.category_group_id = cg.id
      WHERE cg.is_income = 0 AND bm.month > ?
    `).get(month);

    // ➖ Cash overspending from previous month
    const cashOverspendingTotal = getCashOverspendingForMonth(prevMonth);
    const cashOverspending = { total: cashOverspendingTotal };

    // ➕ Left over from previous month (computed as: RTA - this month's components)
    const rta = getReadyToAssign(month);
    const leftOver = rta
      - (inflowThisMonth.total || 0)
      - (positiveCCBalances.total || 0)
      + (assignedThisMonth.total || 0)
      + (cashOverspending.total || 0);

    return {
      readyToAssign: rta,
      leftOverFromPreviousMonth: leftOver,
      inflowThisMonth: inflowThisMonth.total || 0,
      positiveCCBalances: positiveCCBalances.total || 0,
      cashOverspendingPreviousMonth: cashOverspending.total || 0,
      assignedThisMonth: assignedThisMonth.total || 0,
      assignedInFuture: assignedInFuture.total || 0,
    };
  }

  function updateBudgetAssignment(categoryId: number, month: string, assigned: number) {
    // Final safety guard: reject non-finite values, clamp extreme values
    if (!isFinite(assigned)) {
      console.error(`updateBudgetAssignment: rejected non-finite value ${assigned} for category ${categoryId}`);
      return;
    }
    const MAX_ASSIGNED = 100_000_000_000;
    if (Math.abs(assigned) > MAX_ASSIGNED) {
      console.warn(`updateBudgetAssignment: clamping ${assigned} to max ${MAX_ASSIGNED}`);
      assigned = Math.sign(assigned) * MAX_ASSIGNED;
    }

    // Get the current assigned value to compute the delta
    const existing: any = database.prepare(
      'SELECT assigned, available FROM budget_months WHERE category_id = ? AND month = ?'
    ).get(categoryId, month);

    const oldAssigned = existing?.assigned || 0;
    const delta = assigned - oldAssigned;

    if (existing) {
      // Update this month: set new assigned, adjust available by the delta
      database.prepare(`
        UPDATE budget_months 
        SET assigned = ?, available = available + ?
        WHERE category_id = ? AND month = ?
      `).run(assigned, delta, categoryId, month);

      // Clean up ghost entries
      if (assigned === 0) {
        const updated: any = database.prepare(
          'SELECT assigned, activity, available FROM budget_months WHERE category_id = ? AND month = ?'
        ).get(categoryId, month);
        if (updated && updated.assigned === 0 && updated.activity === 0 && updated.available === 0) {
          database.prepare('DELETE FROM budget_months WHERE category_id = ? AND month = ?').run(categoryId, month);
        }
      }
    } else {
      // Only create new entry if assigned is non-zero (don't create ghost entries)
      if (assigned === 0) return;
      const carryforward = computeCarryforward(categoryId, month);
      database.prepare(`
        INSERT INTO budget_months (category_id, month, assigned, activity, available)
        VALUES (?, ?, ?, 0, ?)
      `).run(categoryId, month, assigned, carryforward + assigned);
    }

    // Propagate the delta to ALL subsequent months' available
    if (delta !== 0) {
      database.prepare(`
        UPDATE budget_months
        SET available = available + ?
        WHERE category_id = ? AND month > ?
      `).run(delta, categoryId, month);

      // Clean up ghost entries created by propagation
      database.prepare(`
        DELETE FROM budget_months
        WHERE category_id = ? AND month > ?
          AND assigned = 0 AND activity = 0 AND available = 0
      `).run(categoryId, month);
    }
  }

  function getTransactions(filters?: {
    accountId?: number;
    categoryId?: number;
    startDate?: string;
    endDate?: string;
    limit?: number;
  }) {
    let query = `
      SELECT 
        t.*,
        a.name as account_name,
        c.name as category_name,
        tr.id as transfer_id,
        CASE WHEN t.date > date('now') THEN 1 ELSE 0 END as is_future,
        CASE 
          WHEN tr.id IS NOT NULL THEN
            CASE 
              WHEN tr.from_transaction_id = t.id THEN t_to.account_id
              ELSE t_from.account_id
            END
        END as transfer_account_id,
        CASE 
          WHEN tr.id IS NOT NULL THEN
            CASE 
              WHEN tr.from_transaction_id = t.id THEN a_to.name
              ELSE a_from.name
            END
        END as transfer_account_name
      FROM transactions t
      JOIN accounts a ON t.account_id = a.id
      LEFT JOIN categories c ON t.category_id = c.id
      LEFT JOIN transfers tr ON t.id = tr.from_transaction_id OR t.id = tr.to_transaction_id
      LEFT JOIN transactions t_from ON tr.from_transaction_id = t_from.id
      LEFT JOIN transactions t_to ON tr.to_transaction_id = t_to.id
      LEFT JOIN accounts a_from ON t_from.account_id = a_from.id
      LEFT JOIN accounts a_to ON t_to.account_id = a_to.id
      WHERE 1=1
    `;
    const params: any[] = [];

    if (filters?.accountId) {
      query += ' AND t.account_id = ?';
      params.push(filters.accountId);
    }
    if (filters?.categoryId) {
      query += ' AND t.category_id = ?';
      params.push(filters.categoryId);
    }
    if (filters?.startDate) {
      query += ' AND t.date >= ?';
      params.push(filters.startDate);
    }
    if (filters?.endDate) {
      query += ' AND t.date <= ?';
      params.push(filters.endDate);
    }

    query += ' ORDER BY t.date DESC, t.id DESC';

    if (filters?.limit) {
      query += ' LIMIT ?';
      params.push(filters.limit);
    }

    return database.prepare(query).all(...params);
  }

  function createTransaction(transaction: {
    accountId: number;
    date: string;
    payee?: string;
    categoryId?: number;
    memo?: string;
    outflow?: number;
    inflow?: number;
    cleared?: string;
    flag?: string;
  }) {
    const stmt = database.prepare(`
      INSERT INTO transactions (
        account_id, date, payee, category_id, memo, outflow, inflow, cleared, flag
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);

    return stmt.run(
      transaction.accountId,
      transaction.date,
      transaction.payee || null,
      transaction.categoryId || null,
      transaction.memo || null,
      transaction.outflow || 0,
      transaction.inflow || 0,
      transaction.cleared || 'Uncleared',
      transaction.flag || null
    );
  }

  function updateTransaction(id: number, transaction: Partial<{
    date: string;
    payee: string;
    categoryId: number;
    memo: string;
    outflow: number;
    inflow: number;
    cleared: string;
    flag: string;
  }>) {
    const fields = [];
    const values = [];

    if (transaction.date !== undefined) {
      fields.push('date = ?');
      values.push(transaction.date);
    }
    if (transaction.payee !== undefined) {
      fields.push('payee = ?');
      values.push(transaction.payee);
    }
    if (transaction.categoryId !== undefined) {
      fields.push('category_id = ?');
      values.push(transaction.categoryId);
    }
    if (transaction.memo !== undefined) {
      fields.push('memo = ?');
      values.push(transaction.memo);
    }
    if (transaction.outflow !== undefined) {
      fields.push('outflow = ?');
      values.push(transaction.outflow);
    }
    if (transaction.inflow !== undefined) {
      fields.push('inflow = ?');
      values.push(transaction.inflow);
    }
    if (transaction.cleared !== undefined) {
      fields.push('cleared = ?');
      values.push(transaction.cleared);
    }
    if (transaction.flag !== undefined) {
      fields.push('flag = ?');
      values.push(transaction.flag);
    }

    if (fields.length === 0) return;

    values.push(id);
    const stmt = database.prepare(`UPDATE transactions SET ${fields.join(', ')} WHERE id = ?`);
    return stmt.run(...values);
  }

  function deleteTransaction(id: number) {
    return database.prepare('DELETE FROM transactions WHERE id = ?').run(id);
  }

  function getTransferByTransactionId(transactionId: number): any {
    return database.prepare(`
      SELECT * FROM transfers 
      WHERE from_transaction_id = ? OR to_transaction_id = ?
    `).get(transactionId, transactionId);
  }

  function createTransfer(params: {
    fromAccountId: number;
    toAccountId: number;
    amount: number;
    date: string;
    memo?: string;
    cleared?: string;
  }) {
    const fromAccount: any = database.prepare('SELECT name FROM accounts WHERE id = ?').get(params.fromAccountId);
    const toAccount: any = database.prepare('SELECT name FROM accounts WHERE id = ?').get(params.toAccountId);

    if (!fromAccount || !toAccount) {
      throw new Error('Account not found');
    }

    const result = database.transaction(() => {
      // Create outflow transaction on source account
      const fromResult = database.prepare(`
        INSERT INTO transactions (account_id, date, payee, category_id, memo, outflow, inflow, cleared, flag)
        VALUES (?, ?, ?, NULL, ?, ?, 0, ?, NULL)
      `).run(
        params.fromAccountId,
        params.date,
        `Transfer : ${toAccount.name}`,
        params.memo || null,
        params.amount,
        params.cleared || 'Uncleared'
      );

      // Create inflow transaction on destination account
      const toResult = database.prepare(`
        INSERT INTO transactions (account_id, date, payee, category_id, memo, outflow, inflow, cleared, flag)
        VALUES (?, ?, ?, NULL, ?, 0, ?, ?, NULL)
      `).run(
        params.toAccountId,
        params.date,
        `Transfer : ${fromAccount.name}`,
        params.memo || null,
        params.amount,
        params.cleared || 'Uncleared'
      );

      // Link them in transfers table
      const transferResult = database.prepare(`
        INSERT INTO transfers (from_transaction_id, to_transaction_id)
        VALUES (?, ?)
      `).run(fromResult.lastInsertRowid, toResult.lastInsertRowid);

      // Update both account balances
      updateAccountBalances(params.fromAccountId);
      updateAccountBalances(params.toAccountId);

      return {
        transferId: transferResult.lastInsertRowid,
        fromTransactionId: fromResult.lastInsertRowid,
        toTransactionId: toResult.lastInsertRowid,
      };
    })();

    return result;
  }

  function deleteTransfer(transferId: number): { fromAccountId: number; toAccountId: number } {
    const transfer: any = database.prepare('SELECT * FROM transfers WHERE id = ?').get(transferId);
    if (!transfer) throw new Error('Transfer not found');

    const fromTx: any = database.prepare('SELECT account_id FROM transactions WHERE id = ?').get(transfer.from_transaction_id);
    const toTx: any = database.prepare('SELECT account_id FROM transactions WHERE id = ?').get(transfer.to_transaction_id);

    const fromAccountId = fromTx?.account_id;
    const toAccountId = toTx?.account_id;

    database.transaction(() => {
      database.prepare('DELETE FROM transfers WHERE id = ?').run(transferId);
      database.prepare('DELETE FROM transactions WHERE id = ?').run(transfer.from_transaction_id);
      database.prepare('DELETE FROM transactions WHERE id = ?').run(transfer.to_transaction_id);

      if (fromAccountId) updateAccountBalances(fromAccountId);
      if (toAccountId) updateAccountBalances(toAccountId);
    })();

    return { fromAccountId, toAccountId };
  }

  function getPayees() {
    return database.prepare(`
      SELECT DISTINCT payee 
      FROM transactions 
      WHERE payee IS NOT NULL AND payee != ''
      ORDER BY payee
    `).all().map((row: any) => row.payee);
  }

  function toggleTransactionCleared(id: number) {
    const transaction: any = database.prepare('SELECT cleared FROM transactions WHERE id = ?').get(id);
    if (!transaction) return null;

    if (transaction.cleared === 'Reconciled') return null;

    const newStatus = transaction.cleared === 'Cleared' ? 'Uncleared' : 'Cleared';
    return database.prepare('UPDATE transactions SET cleared = ? WHERE id = ?').run(newStatus, id);
  }

  function reconcileAccount(accountId: number) {
    const result = database.prepare(`
      UPDATE transactions 
      SET cleared = 'Reconciled' 
      WHERE account_id = ? AND cleared = 'Cleared' AND date <= date('now')
    `).run(accountId);

    return result;
  }

  function getReconciliationInfo(accountId: number) {
    const result: any = database.prepare(`
      SELECT 
        COALESCE(SUM(CASE WHEN cleared IN ('Cleared', 'Reconciled') THEN inflow - outflow ELSE 0 END), 0) as cleared_balance,
        COALESCE(SUM(CASE WHEN cleared = 'Reconciled' THEN inflow - outflow ELSE 0 END), 0) as reconciled_balance,
        COALESCE(SUM(CASE WHEN cleared = 'Cleared' THEN inflow - outflow ELSE 0 END), 0) as pending_cleared_balance,
        COUNT(CASE WHEN cleared = 'Cleared' THEN 1 END) as pending_cleared_count
      FROM transactions
      WHERE account_id = ? AND date <= date('now')
    `).get(accountId);

    return result;
  }

  function updateAccountBalances(accountId: number) {
    const result: any = database.prepare(`
      SELECT 
        COALESCE(SUM(inflow - outflow), 0) as balance,
        COALESCE(SUM(CASE WHEN cleared IN ('Cleared', 'Reconciled') THEN inflow - outflow ELSE 0 END), 0) as cleared_balance,
        COALESCE(SUM(CASE WHEN cleared = 'Uncleared' THEN inflow - outflow ELSE 0 END), 0) as uncleared_balance
      FROM transactions
      WHERE account_id = ? AND date <= date('now')
    `).get(accountId);

    return database.prepare(`
      UPDATE accounts 
      SET balance = ?, cleared_balance = ?, uncleared_balance = ?
      WHERE id = ?
    `).run(result.balance, result.cleared_balance, result.uncleared_balance, accountId);
  }

  function updateBudgetActivity(categoryId: number, month: string) {
    const result: any = database.prepare(`
      SELECT COALESCE(SUM(inflow - outflow), 0) as activity
      FROM transactions
      WHERE category_id = ? AND strftime('%Y-%m', date) = ? AND date <= date('now')
    `).get(categoryId, month);

    const activity = result.activity;

    const carryforward = computeCarryforward(categoryId, month);

    const existing: any = database.prepare(`
      SELECT assigned FROM budget_months WHERE category_id = ? AND month = ?
    `).get(categoryId, month);

    const assigned = existing?.assigned || 0;
    const available = carryforward + assigned + activity;

    if (existing) {
      return database.prepare(`
        UPDATE budget_months 
        SET activity = ?, available = ?
        WHERE category_id = ? AND month = ?
      `).run(activity, available, categoryId, month);
    } else {
      return database.prepare(`
        INSERT INTO budget_months (category_id, month, assigned, activity, available)
        VALUES (?, ?, 0, ?, ?)
      `).run(categoryId, month, activity, available);
    }
  }

  function refreshAllBudgetActivity(month: string) {
    // 1. Recalculate all regular categories with transactions in this month
    const categories: any[] = database.prepare(`
      SELECT DISTINCT t.category_id
      FROM transactions t
      JOIN categories c ON t.category_id = c.id
      LEFT JOIN categories cc ON cc.linked_account_id IS NOT NULL AND cc.id = t.category_id
      WHERE strftime('%Y-%m', t.date) = ? AND t.date <= date('now')
        AND t.category_id IS NOT NULL
        AND cc.id IS NULL
    `).all(month);

    for (const cat of categories) {
      updateBudgetActivity(cat.category_id, month);
    }

    // 2. Recalculate all CC Payment categories
    const ccAccounts: any[] = database.prepare(`
      SELECT DISTINCT a.id FROM accounts a WHERE a.type = 'credit'
    `).all();

    for (const acc of ccAccounts) {
      updateCreditCardPaymentBudget(acc.id, month);
    }
  }

  function getTransaction(id: number) {
    return database.prepare(`
      SELECT 
        t.*,
        a.name as account_name,
        c.name as category_name
      FROM transactions t
      JOIN accounts a ON t.account_id = a.id
      LEFT JOIN categories c ON t.category_id = c.id
      WHERE t.id = ?
    `).get(id);
  }

  function updateCategoryName(id: number, name: string) {
    return database.prepare('UPDATE categories SET name = ? WHERE id = ?').run(name, id);
  }

  function updateCategoryGroupOrder(groups: { id: number, sort_order: number }[]) {
    const updateStmt = database.prepare('UPDATE category_groups SET sort_order = ? WHERE id = ?');
    const transaction = database.transaction((groups) => {
      for (const group of groups) {
        updateStmt.run(group.sort_order, group.id);
      }
    });
    return transaction(groups);
  }

  function updateCategoryOrder(categories: { id: number, sort_order: number, category_group_id?: number }[]) {
    const transaction = database.transaction((categories) => {
      for (const cat of categories) {
        if (cat.category_group_id !== undefined) {
          database.prepare('UPDATE categories SET sort_order = ?, category_group_id = ? WHERE id = ?')
            .run(cat.sort_order, cat.category_group_id, cat.id);
        } else {
          database.prepare('UPDATE categories SET sort_order = ? WHERE id = ?')
            .run(cat.sort_order, cat.id);
        }
      }
    });
    return transaction(categories);
  }

  function createCategoryGroup(name: string) {
    const maxOrder = database.prepare('SELECT MAX(sort_order) as maxOrder FROM category_groups').get() as { maxOrder: number };
    const newOrder = (maxOrder.maxOrder || 0) + 1;

    return database.prepare('INSERT INTO category_groups (name, sort_order) VALUES (?, ?)').run(name, newOrder);
  }

  function createCategory(category: { name: string; category_group_id: number; linked_account_id?: number }) {
    const maxOrder = database.prepare('SELECT MAX(sort_order) as maxOrder FROM categories WHERE category_group_id = ?').get(category.category_group_id) as { maxOrder: number };
    const newOrder = (maxOrder.maxOrder || 0) + 1;

    if (category.linked_account_id) {
      return database.prepare('INSERT INTO categories (name, category_group_id, sort_order, linked_account_id) VALUES (?, ?, ?, ?)').run(category.name, category.category_group_id, newOrder, category.linked_account_id);
    }
    return database.prepare('INSERT INTO categories (name, category_group_id, sort_order) VALUES (?, ?, ?)').run(category.name, category.category_group_id, newOrder);
  }

  // ====== Credit Card Payment Functions ======

  function getAccountType(accountId: number): string | null {
    const account: any = database.prepare('SELECT type FROM accounts WHERE id = ?').get(accountId);
    return account?.type || null;
  }

  function getCreditCardPaymentCategory(accountId: number): any {
    return database.prepare(
      'SELECT * FROM categories WHERE linked_account_id = ?'
    ).get(accountId);
  }

  function ensureCreditCardPaymentCategory(accountId: number, accountName: string): any {
    let category = getCreditCardPaymentCategory(accountId);
    if (category) return category;

    let ccGroup: any = database.prepare("SELECT id FROM category_groups WHERE name = 'Credit Card Payments'").get();
    if (!ccGroup) {
      const maxOrder = database.prepare('SELECT MAX(sort_order) as maxOrder FROM category_groups').get() as { maxOrder: number };
      const newOrder = (maxOrder.maxOrder || 0) + 1;
      const result = database.prepare("INSERT INTO category_groups (name, sort_order, hidden, is_income) VALUES ('Credit Card Payments', ?, 0, 0)").run(newOrder);
      ccGroup = { id: result.lastInsertRowid };
    }

    const result = createCategory({
      name: accountName,
      category_group_id: ccGroup.id,
      linked_account_id: accountId,
    });

    return database.prepare('SELECT * FROM categories WHERE id = ?').get(result.lastInsertRowid);
  }

  function updateCreditCardPaymentBudget(accountId: number, month: string) {
    const ccCategory = getCreditCardPaymentCategory(accountId);
    if (!ccCategory) return;

    const categorySpending: any[] = database.prepare(`
      SELECT 
        t.category_id,
        COALESCE(SUM(t.outflow), 0) as cat_outflow,
        COALESCE(SUM(t.inflow), 0) as cat_inflow
      FROM transactions t
      WHERE t.account_id = ? 
        AND strftime('%Y-%m', t.date) = ?
        AND t.date <= date('now')
        AND t.category_id IS NOT NULL
        AND t.category_id != ?
      GROUP BY t.category_id
    `).all(accountId, month, ccCategory.id);

    let fundedActivity = 0;

    for (const catSpend of categorySpending) {
      const netSpending = catSpend.cat_outflow - catSpend.cat_inflow;

      if (netSpending <= 0) {
        fundedActivity += netSpending;
        continue;
      }

      const catBudget: any = database.prepare(`
        SELECT COALESCE(bm.available, 0) as available
        FROM budget_months bm
        WHERE bm.category_id = ? AND bm.month = ?
      `).get(catSpend.category_id, month);

      const currentAvailable = catBudget?.available || 0;
      const availableBefore = currentAvailable + netSpending;

      const funded = Math.min(Math.max(0, availableBefore), netSpending);
      fundedActivity += funded;
    }

    const paymentResult: any = database.prepare(`
      SELECT COALESCE(SUM(t.inflow), 0) as total_payments
      FROM transactions t
      WHERE t.account_id = ?
        AND strftime('%Y-%m', t.date) = ?
        AND t.date <= date('now')
        AND t.category_id IS NULL
        AND t.inflow > 0
    `).get(accountId, month);

    const ccPayments = paymentResult.total_payments || 0;

    const activity = fundedActivity - ccPayments;

    const [y, m] = month.split('-').map(Number);
    const prevDate = new Date(y, m - 2);
    const prevMonth = `${prevDate.getFullYear()}-${String(prevDate.getMonth() + 1).padStart(2, '0')}`;

    const prevCCBudget: any = database.prepare(`
      SELECT available FROM budget_months WHERE category_id = ? AND month = ?
    `).get(ccCategory.id, prevMonth);

    const carryforward = prevCCBudget?.available || 0;

    const existing: any = database.prepare(
      "SELECT assigned FROM budget_months WHERE category_id = ? AND month = ?"
    ).get(ccCategory.id, month);

    const assigned = existing?.assigned || 0;
    const available = carryforward + assigned + activity;

    if (existing) {
      database.prepare(
        "UPDATE budget_months SET activity = ?, available = ? WHERE category_id = ? AND month = ?"
      ).run(activity, available, ccCategory.id, month);
    } else if (activity !== 0) {
      database.prepare(
        "INSERT INTO budget_months (category_id, month, assigned, activity, available) VALUES (?, ?, 0, ?, ?)"
      ).run(ccCategory.id, month, activity, carryforward + activity);
    }
  }

  function isCreditCardAccount(accountId: number): boolean {
    return getAccountType(accountId) === 'credit';
  }

  // ====== Overspending Detection Functions ======

  function getCashOverspendingForMonth(month: string): number {
    const overspentCategories: any[] = database.prepare(`
      SELECT bm.category_id, bm.available
      FROM budget_months bm
      JOIN categories c ON bm.category_id = c.id
      JOIN category_groups cg ON c.category_group_id = cg.id
      WHERE cg.is_income = 0
        AND c.linked_account_id IS NULL
        AND bm.available < 0
        AND bm.month = ?
    `).all(month);

    let totalCashOverspending = 0;

    for (const cat of overspentCategories) {
      const totalOverspent = Math.abs(cat.available);

      const cashActivity: any = database.prepare(`
        SELECT COALESCE(SUM(t.outflow - t.inflow), 0) as total
        FROM transactions t
        JOIN accounts a ON t.account_id = a.id
        WHERE t.category_id = ? AND strftime('%Y-%m', t.date) = ? AND a.type != 'credit'
      `).get(cat.category_id, month);

      const cashSpending = Math.max(0, cashActivity.total || 0);
      const cashOverspending = Math.min(totalOverspent, cashSpending);
      totalCashOverspending += cashOverspending;
    }

    return totalCashOverspending;
  }

  function getBudgetInspectorData(month: string) {
    const [y, m] = month.split('-').map(Number);
    const prevDate = new Date(y, m - 2);
    const prevMonth = `${prevDate.getFullYear()}-${String(prevDate.getMonth() + 1).padStart(2, '0')}`;

    const twelveMonthsAgoDate = new Date(y, m - 13);
    const twelveMonthsAgo = `${twelveMonthsAgoDate.getFullYear()}-${String(twelveMonthsAgoDate.getMonth() + 1).padStart(2, '0')}`;

    // ── Month Summary ──
    const breakdown = getReadyToAssignBreakdown(month);

    const budgetRows = getBudgetForMonth(month);

    let totalAvailable = 0;
    let totalActivity = 0;
    for (const row of budgetRows) {
      if (row.category_id !== null) {
        totalAvailable += row.available || 0;
        totalActivity += row.activity || 0;
      }
    }

    const totalAssigned = breakdown.assignedThisMonth || 0;

    const leftOverFromLastMonth = totalAvailable - totalAssigned - totalActivity;

    // ── Cost to Be Me ──
    const targets = totalAssigned;

    const expectedIncome: any = database.prepare(`
      SELECT COALESCE(AVG(monthly_total), 0) as avg_total
      FROM (
        SELECT strftime('%Y-%m', t.date) as month, SUM(t.inflow - t.outflow) as monthly_total
        FROM transactions t
        JOIN accounts a ON t.account_id = a.id
        JOIN categories c ON t.category_id = c.id
        JOIN category_groups cg ON c.category_group_id = cg.id
        WHERE cg.is_income = 1
          AND a.type != 'credit'
          AND t.date <= date('now')
          AND strftime('%Y-%m', t.date) >= ? AND strftime('%Y-%m', t.date) < ?
        GROUP BY strftime('%Y-%m', t.date)
      )
    `).get(twelveMonthsAgo, month);

    // ── Auto-Assign Calculations ──

    const underfundedRegular: any = database.prepare(`
      SELECT COALESCE(SUM(ABS(bm.available)), 0) as total
      FROM budget_months bm
      JOIN categories c ON bm.category_id = c.id
      JOIN category_groups cg ON c.category_group_id = cg.id
      WHERE cg.is_income = 0 AND c.linked_account_id IS NULL AND bm.available < 0 AND bm.month = ?
    `).get(month);

    const ccShortfall: any = database.prepare(`
      SELECT COALESCE(SUM(
        CASE WHEN ABS(a.balance) > COALESCE(bm.available, 0) AND a.balance < 0
          THEN ABS(a.balance) - COALESCE(bm.available, 0)
          ELSE 0
        END
      ), 0) as total
      FROM accounts a
      LEFT JOIN categories c ON c.linked_account_id = a.id
      LEFT JOIN budget_months bm ON bm.category_id = c.id AND bm.month = ?
      WHERE a.type = 'credit' AND a.closed = 0
    `).get(month);

    const underfundedTotal = (underfundedRegular.total || 0) + (ccShortfall.total || 0);

    const assignedLastMonth: any = database.prepare(`
      SELECT COALESCE(SUM(bm.assigned), 0) as total
      FROM budget_months bm
      JOIN categories c ON bm.category_id = c.id
      JOIN category_groups cg ON c.category_group_id = cg.id
      WHERE cg.is_income = 0 AND c.linked_account_id IS NULL AND bm.month = ?
    `).get(prevMonth);

    const spentLastMonth: any = database.prepare(`
      SELECT COALESCE(SUM(ABS(CASE WHEN bm.activity < 0 THEN bm.activity ELSE 0 END)), 0) as total
      FROM budget_months bm
      JOIN categories c ON bm.category_id = c.id
      JOIN category_groups cg ON c.category_group_id = cg.id
      WHERE cg.is_income = 0 AND c.linked_account_id IS NULL AND bm.month = ?
    `).get(prevMonth);

    const avgAssigned: any = database.prepare(`
      SELECT COALESCE(AVG(monthly_total), 0) as avg_total
      FROM (
        SELECT bm.month, SUM(bm.assigned) as monthly_total
        FROM budget_months bm
        JOIN categories c ON bm.category_id = c.id
        JOIN category_groups cg ON c.category_group_id = cg.id
        WHERE cg.is_income = 0 AND c.linked_account_id IS NULL
          AND bm.month >= ? AND bm.month < ?
        GROUP BY bm.month
      )
    `).get(twelveMonthsAgo, month);

    const avgSpent: any = database.prepare(`
      SELECT COALESCE(AVG(monthly_total), 0) as avg_total
      FROM (
        SELECT bm.month, SUM(ABS(CASE WHEN bm.activity < 0 THEN bm.activity ELSE 0 END)) as monthly_total
        FROM budget_months bm
        JOIN categories c ON bm.category_id = c.id
        JOIN category_groups cg ON c.category_group_id = cg.id
        WHERE cg.is_income = 0 AND c.linked_account_id IS NULL
          AND bm.month >= ? AND bm.month < ?
        GROUP BY bm.month
      )
    `).get(twelveMonthsAgo, month);

    const reduceOverfunding: any = database.prepare(`
      SELECT COALESCE(SUM(bm.available - bm.assigned), 0) as total
      FROM budget_months bm
      JOIN categories c ON bm.category_id = c.id
      JOIN category_groups cg ON c.category_group_id = cg.id
      WHERE cg.is_income = 0 AND c.linked_account_id IS NULL
        AND bm.available > bm.assigned AND bm.assigned > 0 AND bm.activity >= 0
        AND bm.month = ?
    `).get(month);

    const resetAvailable: any = database.prepare(`
      SELECT COALESCE(SUM(bm.available), 0) as total
      FROM budget_months bm
      JOIN categories c ON bm.category_id = c.id
      JOIN category_groups cg ON c.category_group_id = cg.id
      WHERE cg.is_income = 0 AND c.linked_account_id IS NULL
        AND bm.available > 0 AND bm.assigned = 0 AND bm.activity = 0
        AND bm.month = ?
    `).get(month);

    const resetAssigned = totalAssigned;

    // ── Assigned in Future Months ──
    const futureMonths: any[] = database.prepare(`
      SELECT bm.month, SUM(bm.assigned) as total
      FROM budget_months bm
      JOIN categories c ON bm.category_id = c.id
      JOIN category_groups cg ON c.category_group_id = cg.id
      WHERE cg.is_income = 0 AND bm.month > ? AND bm.assigned != 0
      GROUP BY bm.month
      ORDER BY bm.month
    `).all(month);

    return {
      summary: {
        leftOverFromLastMonth,
        assignedThisMonth: totalAssigned,
        activity: totalActivity,
        available: totalAvailable,
      },
      costToBeMe: {
        targets,
        expectedIncome: expectedIncome.avg_total || 0,
      },
      autoAssign: {
        underfunded: underfundedTotal,
        assignedLastMonth: assignedLastMonth.total || 0,
        spentLastMonth: spentLastMonth.total || 0,
        averageAssigned: avgAssigned.avg_total || 0,
        averageSpent: avgSpent.avg_total || 0,
        reduceOverfunding: reduceOverfunding.total || 0,
        resetAvailableAmounts: resetAvailable.total || 0,
        resetAssignedAmounts: resetAssigned,
      },
      futureAssignments: {
        total: breakdown.assignedInFuture,
        months: futureMonths.map(fm => ({
          month: fm.month,
          amount: fm.total,
        })),
      },
    };
  }

  function getOverspendingTypes(month: string): Record<number, 'cash' | 'credit' | null> {
    const result: Record<number, 'cash' | 'credit' | null> = {};

    const overspentCategories: any[] = database.prepare(`
      SELECT bm.category_id, bm.available, c.linked_account_id
      FROM budget_months bm
      JOIN categories c ON bm.category_id = c.id
      JOIN category_groups cg ON c.category_group_id = cg.id
      WHERE cg.is_income = 0
        AND bm.available < 0
        AND bm.month = ?
    `).all(month);

    for (const cat of overspentCategories) {
      if (cat.linked_account_id) {
        result[cat.category_id] = 'credit';
        continue;
      }

      const totalOverspent = Math.abs(cat.available);

      const cashActivity: any = database.prepare(`
        SELECT COALESCE(SUM(t.outflow - t.inflow), 0) as total
        FROM transactions t
        JOIN accounts a ON t.account_id = a.id
        WHERE t.category_id = ? AND strftime('%Y-%m', t.date) = ? AND a.type != 'credit'
      `).get(cat.category_id, month);

      const cashSpending = Math.max(0, cashActivity.total || 0);
      const cashOverspending = Math.min(totalOverspent, cashSpending);
      const creditOverspending = totalOverspent - cashOverspending;

      if (cashOverspending > 0 && creditOverspending === 0) {
        result[cat.category_id] = 'cash';
      } else if (creditOverspending > 0 && cashOverspending === 0) {
        result[cat.category_id] = 'credit';
      } else if (cashOverspending > 0) {
        result[cat.category_id] = 'cash';
      }
    }

    return result;
  }

  return {
    computeCarryforward,
    getAccounts,
    getAccount,
    createAccount,
    updateAccount,
    getCategoryGroups,
    getCategories,
    getBudgetForMonth,
    getReadyToAssign,
    getReadyToAssignBreakdown,
    updateBudgetAssignment,
    getTransactions,
    createTransaction,
    updateTransaction,
    deleteTransaction,
    getTransferByTransactionId,
    createTransfer,
    deleteTransfer,
    getPayees,
    toggleTransactionCleared,
    reconcileAccount,
    getReconciliationInfo,
    updateAccountBalances,
    updateBudgetActivity,
    refreshAllBudgetActivity,
    getTransaction,
    updateCategoryName,
    updateCategoryGroupOrder,
    updateCategoryOrder,
    createCategoryGroup,
    createCategory,
    getAccountType,
    getCreditCardPaymentCategory,
    ensureCreditCardPaymentCategory,
    updateCreditCardPaymentBudget,
    isCreditCardAccount,
    getCashOverspendingForMonth,
    getBudgetInspectorData,
    getOverspendingTypes,
  };
}

// ====== Production singleton ======
// Create all functions bound to the file-based production database.
// The public API is exactly the same as before the refactor.
const fns = createDbFunctions(db);

export const {
  computeCarryforward,
  getAccounts,
  getAccount,
  createAccount,
  updateAccount,
  getCategoryGroups,
  getCategories,
  getBudgetForMonth,
  getReadyToAssign,
  getReadyToAssignBreakdown,
  updateBudgetAssignment,
  getTransactions,
  createTransaction,
  updateTransaction,
  deleteTransaction,
  getTransferByTransactionId,
  createTransfer,
  deleteTransfer,
  getPayees,
  toggleTransactionCleared,
  reconcileAccount,
  getReconciliationInfo,
  updateAccountBalances,
  updateBudgetActivity,
  refreshAllBudgetActivity,
  getTransaction,
  updateCategoryName,
  updateCategoryGroupOrder,
  updateCategoryOrder,
  createCategoryGroup,
  createCategory,
  getAccountType,
  getCreditCardPaymentCategory,
  ensureCreditCardPaymentCategory,
  updateCreditCardPaymentBudget,
  isCreditCardAccount,
  getCashOverspendingForMonth,
  getBudgetInspectorData,
  getOverspendingTypes,
} = fns;
