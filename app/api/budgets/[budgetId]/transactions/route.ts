...
export async function POST(
    ...
    try {
        ...
        if (body.isTransfer) {
            ...
            const result = await createTransfer(budgetId, {
            ...
            const transaction = await getTransaction(budgetId, result.fromTransactionId);
        ...
        const result = await createTransactionAtomic(budgetId, {
...
export async function PUT(
    ...
    try {
        ...
        await updateTransactionAtomic(budgetId, id, original, updateData);
...
export async function DELETE(
    ...
    try {
        ...
        const transfer = await getTransferByTransactionId(budgetId, transactionId);
        if (transfer) {
            await deleteTransfer(budgetId, transfer.id);
            return NextResponse.json({ success: true, deletedTransfer: true });
        }

        // Atomic: delete transaction + update balances + budget + CC payment
        await deleteTransactionAtomic(budgetId, transaction);
...
export async function PATCH(
    ...
    try {
        ...
        if (data.action === 'toggle-cleared') {
            ...
            await toggleClearedAtomic(budgetId, data.id, transaction.accountId);

            const updated = await getTransaction(budgetId, data.id);
        ...
        if (data.action === 'reconcile') {
            ...
            const result = await reconcileAccountAtomic(budgetId, data.accountId);
